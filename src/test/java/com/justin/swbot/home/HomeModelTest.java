...
public class HomeModelTest {
    ...
    public void testLoadData_oldProfilesAreCleared_onReloaded() {
        ...
        final String duplicatedProfile = profiles.stream().reduce(null,
                (duplicated, profile) -> duplicated != null ? duplicated : searchDuplicatedProfile(profiles, profile));
    ...
    public void testLoadData_oldScenariosAreCleared_onReloaded() {
        instanceUnderTest.loadData();
        instanceUnderTest.loadData();
        final List<SimpleImmutableEntry<String, Object>> scenarios = instanceUnderTest.getScenarios();
        final SimpleImmutableEntry<?, ?> duplicatedScenario = scenarios.stream().reduce(null, (duplicated,
                scenario) -> duplicated != null ? duplicated : searchDuplicatedScenario(scenarios, scenario));
    ...
    private String searchDuplicatedProfile(final List<String> profiles, final String profileToSearch) {
        final boolean duplicated = profiles.stream().filter(profile -> profile.equals(profileToSearch)).count() > 1;
     ...
     * @param scenarios
     *            scenario store.
     * @param scenarioToSearch
     *            scenario to search
     * @return the duplicated scenario if exist twice in store
     */
    private SimpleImmutableEntry<String, Object> searchDuplicatedScenario(
            final List<SimpleImmutableEntry<String, Object>> scenarios,
            final SimpleImmutableEntry<String, Object> scenarioToSearch) {
        final Object scenarioId = scenarioToSearch.getKey();
        final boolean duplicated = scenarios.stream().filter(scenario -> scenario.getKey().equals(scenarioId))
                .count() > 1;
